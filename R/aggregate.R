...
aggregate_graphs <- function(graphs, weights = NULL)
{
    ...
    graphs <- lapply(graphs, function(graph)
                  {graph$nodes <- sort(graph$nodes); graph})
    base <- graphs[[1]]
    ...
    if (!same_nodes)
        stop("not all graphs have the same nodes")

    table <- .Call("r_causality_aggregate_graphs", graphs, weights)
    acg <- data.frame(table[[1]], table[[2]], table[[10]], table[[3]],
                      table[[4]], table[[11]], table[[5]], table[[12]],
                      table[[6]], table[[13]], table[[7]], table[[8]],
