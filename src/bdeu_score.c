...
static inline double calc_structure_prior(int n_par, int n_var, int structure_prior);
...
double bdeu_score(dataframe data, int * xy, int n_par, double * fargs, int * iargs) {
  ...
  int * n_j  = alloced_mem + n_x_states * n_x_states;
  ...
  score -= n_x_states * n_x_states * lgamma(cell_prior);
  free(alloced_mem);
  Rprintf("score: %f\n", score);
  return score;
}
